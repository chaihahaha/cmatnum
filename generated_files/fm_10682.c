...
inline int fm_10682(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -16*bmats.A_23_27.data[i][j] + bmats.A_23_28.data[i][j] + bmats.Ax1.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_23_27.data[i][j];
        }
    }
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_27_23.data[i][j]+=17 * m.data[i][j];
        ...
        bmats.C_23_27.data[i][j]+=2 * m.data[i][j];
