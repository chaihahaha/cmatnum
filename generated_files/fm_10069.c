...
inline int fm_10069(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_11_29.data[i][j] - 16*bmats.A_11_30.data[i][j] + bmats.Ax402.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_27_14.data[i][j];
        }
    }
