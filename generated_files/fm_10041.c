...
inline int fm_10041(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_32_29.data[i][j] - 16*bmats.A_32_30.data[i][j] + bmats.Ax1082.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_32_30.data[i][j]+=2 * m.data[i][j];
        bmats.C_32_31.data[i][j]+=1 * m.data[i][j];
        bmats.C_30_32.data[i][j]+=17 * m.data[i][j];
