...
inline int fm_13246(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -16*bmats.A_26_17.data[i][j] + bmats.A_26_18.data[i][j] + bmats.Ax20.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_26_17.data[i][j]+=2 * m.data[i][j];
        ...
        bmats.C_17_26.data[i][j]+=17 * m.data[i][j];
