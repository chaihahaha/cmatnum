...
inline int fm_3712(double_cmat m, pack_mats_22x22 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -11*bmats.A_19_13.data[i][j] + bmats.A_19_14.data[i][j] + bmats.Ax1455.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_13_19.data[i][j]+=2 * m.data[i][j];
