...
inline int fm_10303(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_30_27.data[i][j] - 16*bmats.A_30_28.data[i][j] + bmats.Ax1234.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_30_28.data[i][j];
        }
    }
