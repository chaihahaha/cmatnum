...
inline int fm_10803(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_5_25.data[i][j] + 16*bmats.A_5_26.data[i][j] - bmats.Ax2854.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_10_21.data[i][j]+=17 * m.data[i][j];
        }
    }
