...
inline int fm_11197(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -16*bmats.A_22_23.data[i][j] + bmats.A_22_24.data[i][j] + bmats.Ax354.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_23_22.data[i][j]+=17 * m.data[i][j];
        ...
        bmats.C_22_23.data[i][j]+=2 * m.data[i][j];
