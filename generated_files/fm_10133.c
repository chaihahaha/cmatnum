...
inline int fm_10133(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_18_13.data[i][j] + 16*bmats.A_18_14.data[i][j] - bmats.Ax512.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
        bmats.C_30_2.data[i][j]+=17 * m.data[i][j];
