...
inline int fm_10849(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_32_10.data[i][j] + 16*bmats.A_32_9.data[i][j] - bmats.Ax2801.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
        bmats.C_25_16.data[i][j]+=17 * m.data[i][j];
