...
inline int fm_10921(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_9_10.data[i][j] + 16*bmats.A_9_9.data[i][j] - bmats.Ax2923.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_25_9.data[i][j]+=17 * m.data[i][j];
