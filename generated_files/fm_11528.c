...
inline int fm_11528(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = 16*bmats.A_28_5.data[i][j] - bmats.A_28_6.data[i][j] - bmats.Ax2439.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_28_21.data[i][j];
        }
    }
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
        bmats.C_21_12.data[i][j]+=17 * m.data[i][j];
