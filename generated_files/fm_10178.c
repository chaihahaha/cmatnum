...
inline int fm_10178(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -16*bmats.A_16_13.data[i][j] + bmats.A_16_14.data[i][j] + bmats.Ax1018.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_13_16.data[i][j]+=17 * m.data[i][j];
