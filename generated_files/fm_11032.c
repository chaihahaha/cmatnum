...
inline int fm_11032(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_23_7.data[i][j] - 16*bmats.A_23_8.data[i][j] + bmats.Ax3396.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_8_7.data[i][j]+=17 * m.data[i][j];
