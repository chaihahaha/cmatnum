...
inline int fm_9828(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_16_31.data[i][j] - 16*bmats.A_16_32.data[i][j] + bmats.Ax751.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_32_16.data[i][j];
        }
    }
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_32_32.data[i][j]+=17 * m.data[i][j];
        }
    }
