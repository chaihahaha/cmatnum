...
inline int fm_9899(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_17_31.data[i][j] - 16*bmats.A_17_32.data[i][j] + bmats.Ax667.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_17_32.data[i][j];
        }
    }
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_32_17.data[i][j]+=17 * m.data[i][j];
        ...
        bmats.C_17_32.data[i][j]+=2 * m.data[i][j];
