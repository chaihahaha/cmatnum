...
inline int fm_9751(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_1_1.data[i][j] + 16*bmats.A_1_2.data[i][j] - bmats.A_2_1.data[i][j] + 16*bmats.A_2_2.data[i][j] - bmats.Ax4647.data[i][j] - bmats.Ax4659.data[i][j] - bmats.Ax4661.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.Bx13247.data[i][j];
        }
    }
