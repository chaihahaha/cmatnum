...
inline int fm_11522(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -16*bmats.A_12_5.data[i][j] + bmats.A_12_6.data[i][j] + bmats.Ax2445.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_12_5.data[i][j]+=2 * m.data[i][j];
