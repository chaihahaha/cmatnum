...
inline int fm_4643(double_cmat m, pack_mats_22x22 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = 11*bmats.A_10_4.data[i][j] - bmats.A_10_5.data[i][j] + 11*bmats.A_11_10.data[i][j] - bmats.A_11_11.data[i][j] - bmats.A_4_10.data[i][j] + 11*bmats.A_4_11.data[i][j] - bmats.Ax602.data[i][j] - bmats.Ax773.data[i][j] - bmats.Ax888.data[i][j];
