...
inline int fm_9689(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = 16*bmats.A_15_15.data[i][j] - bmats.A_15_16.data[i][j] + 16*bmats.A_16_15.data[i][j] - bmats.A_16_16.data[i][j] - bmats.Ax463.data[i][j] - bmats.Ax649.data[i][j] - bmats.Ax654.data[i][j];
