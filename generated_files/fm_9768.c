...
inline int fm_9768(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_17_21.data[i][j] + 16*bmats.A_17_22.data[i][j] - bmats.A_22_21.data[i][j] + 16*bmats.A_22_22.data[i][j] - bmats.Ax204.data[i][j] - bmats.Ax3940.data[i][j] - bmats.Ax3960.data[i][j];
