...
inline int fm_11693(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_3_3.data[i][j] - 16*bmats.A_3_4.data[i][j] + bmats.Ax4339.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_4_3.data[i][j]+=17 * m.data[i][j];
