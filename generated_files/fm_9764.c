...
inline int fm_9764(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = 16*bmats.A_18_21.data[i][j] - bmats.A_18_22.data[i][j] + 16*bmats.A_21_21.data[i][j] - bmats.A_21_22.data[i][j] - bmats.Ax3937.data[i][j] - bmats.Ax3943.data[i][j] - bmats.Ax4181.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.Bx13265.data[i][j];
        }
    }
