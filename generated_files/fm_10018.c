...
inline int fm_10018(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -16*bmats.A_21_31.data[i][j] + bmats.A_21_32.data[i][j] + bmats.Ax636.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_21_31.data[i][j];
        }
    }
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_31_21.data[i][j]+=17 * m.data[i][j];
        ...
        bmats.C_21_31.data[i][j]+=2 * m.data[i][j];
