...
inline int fm_2377(double_cmat m, pack_mats_22x22 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_18_12.data[i][j] - 11*bmats.A_18_20.data[i][j] - bmats.A_22_6.data[i][j] + 11*bmats.A_22_7.data[i][j] - bmats.A_9_21.data[i][j] + 11*bmats.A_9_22.data[i][j] - bmats.Ax2857.data[i][j] - bmats.Ax329.data[i][j] - bmats.Ax92.data[i][j];
