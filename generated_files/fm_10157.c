...
inline int fm_10157(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_21_13.data[i][j] + 16*bmats.A_21_14.data[i][j] - bmats.Ax521.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
        bmats.C_30_5.data[i][j]+=17 * m.data[i][j];
