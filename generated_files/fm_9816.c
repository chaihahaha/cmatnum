...
inline int fm_9816(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_14_31.data[i][j] + 16*bmats.A_14_32.data[i][j] - bmats.Ax745.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_16_30.data[i][j]+=17 * m.data[i][j];
        }
    }
