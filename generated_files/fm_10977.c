...
inline int fm_10977(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_16_23.data[i][j] - 16*bmats.A_16_24.data[i][j] + bmats.Ax3424.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_32_8.data[i][j];
        }
    }
