...
inline int fm_13133(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_5_1.data[i][j] - 16*bmats.A_5_2.data[i][j] + bmats.Ax4227.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_2_5.data[i][j]+=17 * m.data[i][j];
