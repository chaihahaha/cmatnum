...
inline int fm_9763(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = 16*bmats.A_2_5.data[i][j] - bmats.A_2_6.data[i][j] + 16*bmats.A_5_5.data[i][j] - bmats.A_5_6.data[i][j] - bmats.Ax3914.data[i][j] - bmats.Ax3920.data[i][j] - bmats.Ax4174.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.Bx13262.data[i][j];
        }
    }
