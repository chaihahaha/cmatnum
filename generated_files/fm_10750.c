...
inline int fm_10750(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_14_25.data[i][j] - 16*bmats.A_14_26.data[i][j] + bmats.Ax1489.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_30_10.data[i][j];
        }
    }
