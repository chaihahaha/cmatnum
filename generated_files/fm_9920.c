...
inline int fm_9920(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_19_15.data[i][j] + 16*bmats.A_19_16.data[i][j] - bmats.Ax706.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
        bmats.C_32_3.data[i][j]+=17 * m.data[i][j];
