...
inline int fm_11031(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_23_23.data[i][j] - 16*bmats.A_23_24.data[i][j] + bmats.Ax357.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_23_24.data[i][j];
        }
    }
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_24_23.data[i][j]+=17 * m.data[i][j];
        ...
        bmats.C_23_24.data[i][j]+=2 * m.data[i][j];
