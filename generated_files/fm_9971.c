...
inline int fm_9971(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -16*bmats.A_23_31.data[i][j] + bmats.A_23_32.data[i][j] + bmats.Ax618.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_31_23.data[i][j]+=17 * m.data[i][j];
        ...
        bmats.C_23_31.data[i][j]+=2 * m.data[i][j];
