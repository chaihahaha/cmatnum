...
inline int fm_11843(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_16_1.data[i][j] - 16*bmats.A_16_2.data[i][j] + bmats.Ax4644.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_16_2.data[i][j]+=2 * m.data[i][j];
        ...
        bmats.C_2_16.data[i][j]+=17 * m.data[i][j];
