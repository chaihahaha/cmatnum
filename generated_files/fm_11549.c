...
inline int fm_11549(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -16*bmats.A_7_5.data[i][j] + bmats.A_7_6.data[i][j] + bmats.Ax3828.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_5_7.data[i][j]+=17 * m.data[i][j];
