...
inline int fm_10186(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_1_13.data[i][j] - 16*bmats.A_1_14.data[i][j] + bmats.Ax1012.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_14_1.data[i][j]+=17 * m.data[i][j];
        ...
        bmats.C_1_14.data[i][j]+=2 * m.data[i][j];
