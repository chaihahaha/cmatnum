...
inline int fm_11576(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -16*bmats.A_18_21.data[i][j] + bmats.A_18_22.data[i][j] + bmats.Ax3943.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_21_18.data[i][j]+=17 * m.data[i][j];
