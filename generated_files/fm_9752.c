...
inline int fm_9752(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_17_17.data[i][j] + 16*bmats.A_17_18.data[i][j] - bmats.A_18_17.data[i][j] + 16*bmats.A_18_18.data[i][j] - bmats.Ax158.data[i][j] - bmats.Ax4655.data[i][j] - bmats.Ax4666.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.Bx13249.data[i][j];
        }
    }
