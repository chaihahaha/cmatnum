...
inline int fm_3300(double_cmat m, pack_mats_22x22 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_2_1.data[i][j] + bmats.A_2_10.data[i][j] + bmats.A_2_11.data[i][j] + bmats.A_2_2.data[i][j] + bmats.A_2_3.data[i][j] + bmats.A_2_4.data[i][j] + bmats.A_2_5.data[i][j] + bmats.A_2_6.data[i][j] - 11*bmats.A_2_7.data[i][j] + bmats.A_2_8.data[i][j] + bmats.A_2_9.data[i][j];
            tmp0.data[i][j] *= dnum12;
            tmp1.data[i][j] = bmats.B_2_7.data[i][j];
        }
    }
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_2_7.data[i][j]+=12 * m.data[i][j];
        ...
        bmats.C_7_2.data[i][j]+=2 * m.data[i][j];
