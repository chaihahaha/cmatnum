...
inline int fm_9966(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -16*bmats.A_30_31.data[i][j] + bmats.A_30_32.data[i][j] + bmats.Ax604.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_30_31.data[i][j];
        }
    }
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_31_30.data[i][j]+=17 * m.data[i][j];
        ...
        bmats.C_30_31.data[i][j]+=2 * m.data[i][j];
