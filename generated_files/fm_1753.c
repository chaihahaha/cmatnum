...
inline int fm_1753(double_cmat m, pack_mats_22x22 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_13_10.data[i][j] + 11*bmats.A_13_11.data[i][j] - 11*bmats.A_1_2.data[i][j] + bmats.A_1_3.data[i][j] - bmats.Ax1770.data[i][j] - bmats.Ax225.data[i][j] - bmats.Ax2464.data[i][j];
