...
inline int fm_10639(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -16*bmats.A_26_27.data[i][j] + bmats.A_26_28.data[i][j] + bmats.Ax152.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_26_27.data[i][j];
        }
    }
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_27_26.data[i][j]+=17 * m.data[i][j];
        ...
        bmats.C_26_27.data[i][j]+=2 * m.data[i][j];
