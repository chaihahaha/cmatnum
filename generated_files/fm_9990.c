...
inline int fm_9990(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = 16*bmats.A_9_15.data[i][j] - bmats.A_9_16.data[i][j] - bmats.Ax579.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_25_15.data[i][j];
        }
    }
