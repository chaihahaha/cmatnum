...
inline int fm_1154(double_cmat m, pack_mats_22x22 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_18_10.data[i][j] + 11*bmats.A_18_11.data[i][j] + bmats.A_1_6.data[i][j] - 11*bmats.A_1_7.data[i][j] - bmats.Ax1770.data[i][j] - bmats.Ax2291.data[i][j] - bmats.Ax639.data[i][j];
