...
inline int fm_11555(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = 16*bmats.A_23_5.data[i][j] - bmats.A_23_6.data[i][j] - bmats.Ax3822.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_23_21.data[i][j];
        }
    }
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
        bmats.C_21_7.data[i][j]+=17 * m.data[i][j];
