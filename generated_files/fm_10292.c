...
inline int fm_10292(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -16*bmats.A_5_13.data[i][j] + bmats.A_5_14.data[i][j] + bmats.Ax1048.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_5_13.data[i][j]+=2 * m.data[i][j];
