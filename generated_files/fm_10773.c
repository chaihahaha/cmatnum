...
inline int fm_10773(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = 16*bmats.A_24_10.data[i][j] - bmats.A_24_9.data[i][j] - bmats.Ax2807.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
        bmats.C_26_8.data[i][j]+=17 * m.data[i][j];
