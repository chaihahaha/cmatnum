...
inline int fm_2735(double_cmat m, pack_mats_22x22 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = 11*bmats.A_11_21.data[i][j] - bmats.A_11_22.data[i][j] + bmats.A_15_12.data[i][j] + bmats.A_15_13.data[i][j] + bmats.A_15_14.data[i][j] + bmats.A_15_15.data[i][j] + bmats.A_15_16.data[i][j] + bmats.A_15_17.data[i][j] + bmats.A_15_18.data[i][j] + bmats.A_15_19.data[i][j] + bmats.A_15_20.data[i][j] + bmats.A_15_21.data[i][j] - 11*bmats.A_15_22.data[i][j] + 11*bmats.A_21_4.data[i][j] - bmats.A_21_5.data[i][j] - bmats.Ax561.data[i][j] - bmats.Ax932.data[i][j];
            tmp0.data[i][j] *= dnum12;
            tmp1.data[i][j] = bmats.B_21_15.data[i][j] + bmats.B_22_10.data[i][j] + bmats.B_4_22.data[i][j];
        }
    }
