...
inline int fm_11467(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = 16*bmats.A_13_5.data[i][j] - bmats.A_13_6.data[i][j] - bmats.Ax2086.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_29_5.data[i][j];
        }
    }
