...
inline int fm_9738(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_27_15.data[i][j] - bmats.A_27_16.data[i][j] - bmats.A_27_31.data[i][j] - bmats.A_27_32.data[i][j] - bmats.Ax609.data[i][j] - bmats.Ax685.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = -bmats.B_27_17.data[i][j] - bmats.B_27_18.data[i][j] - bmats.B_27_23.data[i][j] - bmats.Bx7425.data[i][j];
        }
    }
