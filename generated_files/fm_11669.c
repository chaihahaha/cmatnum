...
inline int fm_11669(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_23_19.data[i][j] + 16*bmats.A_23_20.data[i][j] - bmats.Ax88.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_4_23.data[i][j]+=17 * m.data[i][j];
        }
    }
