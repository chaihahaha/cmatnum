...
inline int fm_3005(double_cmat m, pack_mats_22x22 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -11*bmats.A_16_21.data[i][j] + bmats.A_16_22.data[i][j] + bmats.Ax828.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_21_16.data[i][j]+=2 * m.data[i][j];
