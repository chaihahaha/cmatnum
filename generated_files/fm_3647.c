...
inline int fm_3647(double_cmat m, pack_mats_22x22 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -11*bmats.A_20_13.data[i][j] + bmats.A_20_14.data[i][j] + bmats.Ax1482.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_13_20.data[i][j]+=2 * m.data[i][j];
