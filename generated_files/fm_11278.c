...
inline int fm_11278(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_16_5.data[i][j] - 16*bmats.A_16_6.data[i][j] + bmats.Ax3911.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_16_6.data[i][j]+=2 * m.data[i][j];
        ...
        bmats.C_6_16.data[i][j]+=17 * m.data[i][j];
