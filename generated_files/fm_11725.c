...
inline int fm_11725(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = 16*bmats.A_31_3.data[i][j] - bmats.A_31_4.data[i][j] - bmats.Ax913.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
        bmats.C_19_15.data[i][j]+=17 * m.data[i][j];
