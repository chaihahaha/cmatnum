...
inline int fm_10738(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_12_25.data[i][j] + 16*bmats.A_12_26.data[i][j] - bmats.Ax2387.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_10_28.data[i][j]+=17 * m.data[i][j];
        }
    }
