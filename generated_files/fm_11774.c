...
inline int fm_11774(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = 16*bmats.A_7_3.data[i][j] - bmats.A_7_4.data[i][j] - bmats.Ax3813.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_23_3.data[i][j];
        }
    }
