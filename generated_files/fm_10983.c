...
inline int fm_10983(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -16*bmats.A_17_25.data[i][j] + bmats.A_17_26.data[i][j] + bmats.Ax239.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_25_17.data[i][j]+=17 * m.data[i][j];
        ...
        bmats.C_17_25.data[i][j]+=2 * m.data[i][j];
