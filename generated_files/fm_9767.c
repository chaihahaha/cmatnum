...
inline int fm_9767(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_1_5.data[i][j] + 16*bmats.A_1_6.data[i][j] - bmats.A_6_5.data[i][j] + 16*bmats.A_6_6.data[i][j] - bmats.Ax3917.data[i][j] - bmats.Ax3953.data[i][j] - bmats.Ax3955.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.Bx13268.data[i][j];
        }
    }
