...
inline int fm_13177(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_19_1.data[i][j] + 16*bmats.A_19_2.data[i][j] - bmats.Ax4586.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
        bmats.C_18_3.data[i][j]+=17 * m.data[i][j];
