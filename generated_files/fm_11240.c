...
inline int fm_11240(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -16*bmats.A_19_7.data[i][j] + bmats.A_19_8.data[i][j] + bmats.Ax3402.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_7_3.data[i][j]+=17 * m.data[i][j];
