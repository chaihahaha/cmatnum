...
inline int fm_3037(double_cmat m, pack_mats_22x22 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_11_8.data[i][j] - 11*bmats.A_11_9.data[i][j] + bmats.Ax3128.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_9_11.data[i][j]+=2 * m.data[i][j];
