...
inline int fm_9693(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_12_15.data[i][j] + 16*bmats.A_12_16.data[i][j] - bmats.A_16_15.data[i][j] + 16*bmats.A_16_16.data[i][j] - bmats.Ax463.data[i][j] - bmats.Ax565.data[i][j] - bmats.Ax7791.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.Bx13235.data[i][j];
        }
    }
