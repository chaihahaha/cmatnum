...
inline int fm_12995(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -16*bmats.A_28_23.data[i][j] + bmats.A_28_24.data[i][j] + bmats.A_28_28.data[i][j] + bmats.A_28_30.data[i][j] + bmats.A_28_31.data[i][j] + bmats.A_28_32.data[i][j] - bmats.A_29_11.data[i][j] + 16*bmats.A_29_12.data[i][j] + 16*bmats.A_7_29.data[i][j] - bmats.A_7_30.data[i][j] - bmats.Ax1981.data[i][j] - bmats.Ax417.data[i][j] - bmats.Ax7451.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_12_23.data[i][j] + bmats.B_23_13.data[i][j] + bmats.B_29_28.data[i][j];
        }
    }
