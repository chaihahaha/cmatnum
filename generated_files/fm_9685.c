...
inline int fm_9685(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_14_13.data[i][j] + 16*bmats.A_14_14.data[i][j] - bmats.A_1_13.data[i][j] + 16*bmats.A_1_14.data[i][j] - bmats.Ax1012.data[i][j] - bmats.Ax1056.data[i][j] - bmats.Ax965.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.Bx13212.data[i][j];
        }
    }
