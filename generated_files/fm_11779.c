...
inline int fm_11779(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -16*bmats.A_24_19.data[i][j] + bmats.A_24_20.data[i][j] + bmats.Ax105.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_24_19.data[i][j]+=2 * m.data[i][j];
        ...
        bmats.C_19_24.data[i][j]+=17 * m.data[i][j];
