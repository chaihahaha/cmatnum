...
inline int fm_10217(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -16*bmats.A_12_29.data[i][j] + bmats.A_12_30.data[i][j] + bmats.Ax405.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_28_13.data[i][j];
        }
    }
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_29_28.data[i][j]+=17 * m.data[i][j];
