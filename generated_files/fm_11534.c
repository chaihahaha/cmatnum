...
inline int fm_11534(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -16*bmats.A_21_21.data[i][j] + bmats.A_21_22.data[i][j] + bmats.Ax3937.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_21_21.data[i][j]+=10 * m.data[i][j];
