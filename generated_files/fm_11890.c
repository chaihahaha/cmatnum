...
inline int fm_11890(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_15_1.data[i][j] - 16*bmats.A_15_2.data[i][j] + bmats.Ax742.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_15_2.data[i][j]+=2 * m.data[i][j];
        ...
        bmats.C_2_15.data[i][j]+=17 * m.data[i][j];
