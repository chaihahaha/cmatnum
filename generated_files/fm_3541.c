...
inline int fm_3541(double_cmat m, pack_mats_22x22 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_12_1.data[i][j] + bmats.A_12_10.data[i][j] + bmats.A_12_11.data[i][j] + bmats.A_12_2.data[i][j] + bmats.A_12_3.data[i][j] - 11*bmats.A_12_4.data[i][j] + bmats.A_12_5.data[i][j] + bmats.A_12_6.data[i][j] + bmats.A_12_7.data[i][j] + bmats.A_12_8.data[i][j] + bmats.A_12_9.data[i][j];
            tmp0.data[i][j] *= dnum12;
            tmp1.data[i][j] = bmats.B_1_15.data[i][j];
        }
    }
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
        bmats.C_1_4.data[i][j]+=12 * m.data[i][j];
