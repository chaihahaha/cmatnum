...
inline int fm_13171(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_3_1.data[i][j] - 16*bmats.A_3_2.data[i][j] + bmats.Ax4592.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_2_3.data[i][j]+=17 * m.data[i][j];
