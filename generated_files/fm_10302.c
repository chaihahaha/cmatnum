...
inline int fm_10302(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_14_11.data[i][j] - 16*bmats.A_14_12.data[i][j] + bmats.Ax1255.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_12_14.data[i][j]+=17 * m.data[i][j];
