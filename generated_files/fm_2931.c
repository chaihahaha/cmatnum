...
inline int fm_2931(double_cmat m, pack_mats_22x22 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_14_21.data[i][j] - 11*bmats.A_14_22.data[i][j] + bmats.Ax316.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_22_14.data[i][j]+=2 * m.data[i][j];
