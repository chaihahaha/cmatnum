...
inline int fm_12959(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_28_23.data[i][j] - 16*bmats.A_28_24.data[i][j] + bmats.A_28_28.data[i][j] + bmats.A_28_30.data[i][j] + bmats.A_28_31.data[i][j] + bmats.A_28_32.data[i][j] - bmats.A_30_11.data[i][j] + 16*bmats.A_30_12.data[i][j] - bmats.A_8_29.data[i][j] + 16*bmats.A_8_30.data[i][j] - bmats.Ax1224.data[i][j] - bmats.Ax420.data[i][j] - bmats.Ax7451.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_12_24.data[i][j] + bmats.B_24_14.data[i][j] + bmats.B_30_28.data[i][j];
        }
    }
