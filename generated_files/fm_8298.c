...
inline int fm_8298(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_24_18.data[i][j] + bmats.A_24_20.data[i][j] - 16*bmats.A_24_21.data[i][j] + bmats.A_24_22.data[i][j] + bmats.A_24_24.data[i][j] + bmats.A_24_26.data[i][j] + bmats.A_24_27.data[i][j] + bmats.A_24_28.data[i][j] - bmats.A_25_7.data[i][j] + 16*bmats.A_25_8.data[i][j] + 16*bmats.A_5_25.data[i][j] - bmats.A_5_26.data[i][j] - bmats.Ax2854.data[i][j] - bmats.Ax3322.data[i][j] - bmats.Ax6953.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_21_9.data[i][j] + bmats.B_25_24.data[i][j] + bmats.B_8_21.data[i][j];
        }
    }
