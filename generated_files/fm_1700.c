...
inline int fm_1700(double_cmat m, pack_mats_22x22 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -bmats.A_1_17.data[i][j] + 11*bmats.A_1_18.data[i][j] - 11*bmats.A_7_2.data[i][j] + bmats.A_7_3.data[i][j] + bmats.A_7_7.data[i][j] - bmats.Ax159.data[i][j] - bmats.Ax1762.data[i][j] - bmats.Ax2441.data[i][j];
