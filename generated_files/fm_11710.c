...
inline int fm_11710(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = bmats.A_21_19.data[i][j] - 16*bmats.A_21_20.data[i][j] + bmats.Ax187.data[i][j];
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_20_21.data[i][j]+=17 * m.data[i][j];
