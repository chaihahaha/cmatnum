...
inline int fm_9942(double_cmat m, pack_mats_32x32 bmats) {
    ...
    for (int i=0; i<BL; i++) {
        for (int j=0; j<BL; j++) {
            tmp0.data[i][j] = -16*bmats.A_27_31.data[i][j] + bmats.A_27_32.data[i][j] + bmats.Ax609.data[i][j];
            tmp0.data[i][j] *= dnum17;
            tmp1.data[i][j] = bmats.B_27_31.data[i][j];
        }
    }
    ...
    for (int i=0; i<BL; i++) {
        ...
        bmats.C_31_27.data[i][j]+=17 * m.data[i][j];
        ...
        bmats.C_27_31.data[i][j]+=2 * m.data[i][j];
