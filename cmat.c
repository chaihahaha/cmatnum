...
int create_int_matrix(shape_uint shape[2], int_cmat* p_new_mat) {
    ...
    for (shape_uint i = 0; i < shape[0]; i++) {
        (*p_new_mat).data[i] = &(*p_new_mat).arena[i * shape[1]];
    }
    for (shape_uint j = 0; j < N; j++) {
        (*p_new_mat).arena[j] = 0;
    }
    return 0;
}
...
int create_int_matrix_from_array(shape_uint shape[2], int* array, shape_uint array_length, shape_uint offset[2], int_cmat* p_new_mat) {
    ...
    for (shape_uint i = offset[0]; i < offset[0] + shape[0]; i++) {
...
int_cmat slice_int_matrix(int_cmat mat, shape_int slice0[2], shape_int slice1[2]) {
    ...
    int_cmat new_mat;
    shape_uint shape[2] = {slice0[1] - slice0[0], slice1[1] - slice1[0]};
    ...
    shape_uint offset[2] = {mat.offset[0] + slice0[0], mat.offset[1] + slice1[0]};
    new_mat.offset[0] = offset[0];
    new_mat.offset[1] = offset[1];
    for (shape_uint i = 0; i < shape[0]; i++) {
        shape_uint arenai = i + offset[0];
...
int create_slice_double_matrix_contiguous(double_cmat *dst, double_cmat mat, shape_int slice0[2], shape_int slice1[2]) {
    ...
    shape_uint dsti = 0;
    for (shape_uint i = slice0[0]; i < slice0[1]; i++) {
...
int create_double_contiguous_from_slice(double_cmat *dest, double_cmat *src) {
    ...
    shape_uint i, j;
    shape_uint rows = src->shape[0];
    shape_uint cols = src->shape[1];
...
int assign_double_clone(double_cmat m1, double_cmat m2) {
    ...
    shape_uint N = m2.shape[0]*m2.shape[1];
    if (iscontig) {
        cblas_dcopy(N, m2.data[0], 1, m1.data[0], 1);
    }
    else {
        for (shape_uint i = 0; i < m2.shape[0]; i++) {
...
int matadd_int(int_cmat m3, int_cmat m1, int_cmat m2) {
    ...
    for (shape_uint i = 0; i < m2.shape[0]; i++) {
        for (shape_uint j = 0; j < m2.shape[1]; j++) {
...
int matsub_double(double_cmat m3, double_cmat m1, double_cmat m2) {
    ...
    if (m1.data[0] == m3.data[0]) {
        ...
        if (iscontig) {
            ...
            cblas_dcopy(N, m2.data[0], 1, tmp2, 1);
            cblas_dcopy(N, m1.data[0], 1, m3.data[0], 1);
...
int matneg_double(double_cmat m2, double_cmat m1) {
    ...
    for (shape_uint i = 0; i < m2.shape[0]*m2.shape[1]; i++) {
            m2.data[0][i] = -m1.data[0][i];
    }
    return 0;
}
...
int print_int_matrix(int_cmat mat) {
    printf("[\n");
    for (shape_uint row = 0; row < mat.shape[0]; row++) {
        for (shape_uint col = 0; col < mat.shape[1]; col++) {
