...
struct IntCMat slice_int_matrix(struct IntCMat* p_mat, int slice0[2], int slice1[2]) {
    ...
    if (slice0[1] < 0) {
        slice0[1] += (*p_mat).shape[0];
    }
    if (slice1[1] < 0) {
        slice1[1] += (*p_mat).shape[1];
    }
    if (slice0[0] >= slice0[1] || slice1[0] >= slice1[1]) {
        return empty_mat;
    }
    slice0[1] = MIN(slice0[1], (*p_mat).shape[0]);
    slice1[1] = MIN(slice1[1], (*p_mat).shape[1]);
    struct IntCMat new_mat;
    int shape[2] = {slice0[1] - slice0[0], slice1[1] - slice1[0]};
    new_mat.data = (int **)malloc(shape[0] * sizeof(int *));
    new_mat.arena = (*p_mat).arena;
    new_mat.shape[0] = shape[0];
    new_mat.shape[1] = shape[1];
    new_mat.arena_shape[0] = (*p_mat).arena_shape[0];
    new_mat.arena_shape[1] = (*p_mat).arena_shape[1];
    int offset[2] = {(*p_mat).offset[0] + slice0[0], (*p_mat).offset[1] + slice1[0]};
    new_mat.offset[0] = offset[0];
    new_mat.offset[1] = offset[1];
    for (int i = 0; i < shape[0]; i++) {
        int arenai = i + offset[0];
        new_mat.data[i] = &(*p_mat).arena[arenai * new_mat.arena_shape[1] + offset[1]];
    }
    return new_mat;
}
...
int print_int_matrix(struct IntCMat* p_mat) {
    printf("[\n");
    for (int row = 0; row < (*p_mat).shape[0]; row++) {
        for (int col = 0; col < (*p_mat).shape[1]; col++) {
            printf("%d, ", (*p_mat).data[row][col]);
        }
        printf("\n");
    }
    printf("]\n");
