...
def generate_fm_source_files(A_eval_order, A_reduced_exprs, B_reduced_exprs, B_replacements):
    ...
    for fm_index_1 in range(15136):
        ...
        for j in range(n_A_mats):
            content += f"""
        cblas_daxpy(NS, dnum17*{A_coeffs[j]}, {A_names[j]}.data[0], 1, tmp0.data[0], 1);
"""
        ...
        for j in range(n_B_mats):
            content += f"""
        cblas_daxpy(NS, {B_coeffs[j]}, {B_names[j]}.data[0], 1, tmp1.data[0], 1);
"""
...
def generate_fAxxeval_source_files(A_eval_order, A_reduced_exprs, B_reduced_exprs, B_replacements):
    ...
    for i in range(N_Axx_eval):
        ...
        for j in range(n_A_mats):
            content += f"""
        cblas_daxpy(NS, {A_coeffs[j]}, {A_names[j]}.data[0], 1, {idf}.data[0], 1);
"""
        Axxi2code[i] = content
    return Axxi2code
...
def generate_fBx_source_files(A_eval_order, A_reduced_exprs, B_reduced_exprs, B_replacements):
    ...
    for i,j in B_replacements:
        ...
        for j in range(n_B_mats):
            content += f"""
        cblas_daxpy(NS, {B_coeffs[j]}, {B_names[j]}.data[0], 1, {idf}.data[0], 1);
"""
